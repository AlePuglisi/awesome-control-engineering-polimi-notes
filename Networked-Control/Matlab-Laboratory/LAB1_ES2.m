...
for k=1:length(t) 
    y_free(k) = H* (F)^k*x0;
    y_free(k)
    y_forced(k)= H/(eye(n)-F)*(eye(n)-F^k)*G*u;
end
...
% C) Compute the equilibrium point of the system when   𝑢(𝑘)=1 for all 𝑘∈ℕ.

eq_discrete = H/(eye(n)-F)*G*u

% D) Set  up  and  solve  an  LMI  feasibility  problem  for  
