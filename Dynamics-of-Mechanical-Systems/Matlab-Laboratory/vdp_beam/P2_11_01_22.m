...
for imode=1:nmodes
    ...
    ind1=1;
    modal_stiff(imode)=modal_stiff(imode)+w(imode,ind1)^2*k;
end
...
%% FRF using the modal approach - set proper values for ind_input & ind_output
    ...
    vett_w=zeros(1,nf);
    i=sqrt(-1);
    ind_input =floor(length(x)/2) + 1; % input: force at mid distance 
    ind_output=ind_input; % output: displacement in the left of the system
    for imode=1:nmodes
