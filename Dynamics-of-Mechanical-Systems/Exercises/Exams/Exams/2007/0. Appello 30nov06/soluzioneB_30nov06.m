...
% risposta in frequenza: forza unitaria carrello

ff=[0:0.01:20];        %definizione vettore delle frequenze
ome=2*pi*ff;
vett_F=[1;0;0];             %definizione vettore forzanti
for i=1:length(ff)
    A=-ome(i)^2*mtx_m+j*ome(i)*mtx_r+mtx_k;
    x=inv(A)*vett_F;
    mod1(i)=abs(x(2));              % modulo rotazione disco
    fas1(i)=angle(x(2));            % fase rotazione disco
    mod2(i)=abs(x(3));              % modulo rotazione asta
    fas2(i)=angle(x(3));            % fase rotazione asta
    Rv=-2*L*x(3)*(k3+j*ome(i)*r3);  % reazione vincolare
    mod3(i)=abs(Rv);                % modulo rotazione asta
    fas3(i)=angle(Rv);              % fase rotazione asta
end
...
% risposta in frequenza: forza unitaria asta

vett_F=[0;0;2*L];             
for i=1:length(ff)
    A=-ome(i)^2*mtx_m+j*ome(i)*mtx_r+mtx_k;
    x=inv(A)*vett_F;
    xc=x(1)+R1*x(2);            %spostamento orizzontale centro del disco
    xb=2*L*x(3);                %spostamento orizzontale punto B
    mod1(i)=abs(xc);           
    fas1(i)=angle(xc);          
    mod2(i)=abs(xb);              
    fas2(i)=angle(xb);            
