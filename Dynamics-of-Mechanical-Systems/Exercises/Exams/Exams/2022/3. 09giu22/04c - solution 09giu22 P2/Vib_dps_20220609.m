...
% draw the modal shape function
    ...
    modal_mass(imode)=modal_mass(imode)+M*w(imode,1)^2+J*wp(imode,1)^2;
    funct=EI*wpp(imode,:).*wpp(imode,:);
    modal_stiff(imode)=trapz(x,funct);
    modal_stiff(imode)=modal_stiff(imode)+K*w(imode,1)^2;
end
...
% modal approach 
    ...
    vett_w=zeros(1,nf);
    i=sqrt(-1);
    ind_input =floor(length(x)/2)+1; % input: 1/2 L 
    ind_output=1;                    % output: left end
