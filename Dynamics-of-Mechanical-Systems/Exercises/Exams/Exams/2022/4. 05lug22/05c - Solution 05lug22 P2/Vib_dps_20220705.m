...
% draw the modal shape function
    ...
    ind_mass=floor(length(x)/2)+1; % central position
    modal_mass(imode)=modal_mass(imode)+M*w(imode,ind_mass)^2+J*wp(imode,ind_mass)^2;
...
% modal approach 
    ...
    vett_w=zeros(1,nf);
    i=sqrt(-1);
    ind_input =floor(length(x)/2)+1; % input: centre 
    ind_output=ind_input;                    % output: centre
