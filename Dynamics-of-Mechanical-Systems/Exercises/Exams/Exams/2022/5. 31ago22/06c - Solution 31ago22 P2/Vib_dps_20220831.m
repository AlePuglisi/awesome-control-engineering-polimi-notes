% tema d'esame 31/08/2022
close all
clear all

global EI L m 
%global M J if additional mass
%global k kt if spring liear or torsional
global namefunc
...
% draw the modal shape function
    ...
    %torsindex= ; where is the torsional spring
    %linindex= ; where is the linear spring
    %modal_stiff(imode)=modal_stiff(imode)+ kt*wp(imode,torsionindex)^2+k*w(imode,linindex)^2;
end
...
% modal approach 
    ...
    vett_w=zeros(1,nf);
    i=sqrt(-1);
    ind_input =; % input: where act the forcing effect 
    ind_output=; % output: where I want to measure vertical vibration
