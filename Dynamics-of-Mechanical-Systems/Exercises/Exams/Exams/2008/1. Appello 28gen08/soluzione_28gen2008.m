...
mtx_kg  =  zeros(3);mtx_kg(3,3)=M4*g*L;
disp(' ')
...
mtx_Jm  =  [1/R1 0 0; 1 0 0;1 R3 0; 0 1 0; 1 R3 L]
disp(' ')

disp('jacobiano matrice di rigidezza')
mtx_Jel  =  [2 0 0;0 R3 0]
disp(' ')
...
for i=1:length(ff)
    A=-ome(i)^2*mtx_m+j*ome(i)*mtx_r+mtx_k;
    x=inv(A)*vett_F;
    Ho=-ome(i)^2*((M2+M3+M4)*x(1)+(M3+M4)*R3*x(2)+M4*L*x(3));
    modHo(i)=abs(Ho);
    fasHo(i)=angle(Ho);
end
