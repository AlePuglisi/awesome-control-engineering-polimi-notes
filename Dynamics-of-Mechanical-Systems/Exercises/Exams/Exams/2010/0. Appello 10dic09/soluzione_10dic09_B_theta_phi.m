...
mtx_Jm  =  [-2*H 0 1;1 0 0;-3*H H 1;0 1 0]
disp(' ')

disp('jacobiano matrice di rigidezza')
mtx_Jel  =  [-2*H 0 1;-2*H 2*H 0;3*H -H -1]
disp(' ')
...
ome=2*pi*ff;
vett_F=[0;1];             %definizione vettore forzanti
for i=1:length(ff)
...
for k=1:N/2
    ...
    xc=-3*H*x(1)+H*x(2)+mody(k)*exp(j*fasy(k));
    yimp=yimp+mody(k)*cos(ome*tempo+fasy(k)); 
    xper=xper+abs(xc)*cos(ome*tempo+angle(xc)); 
    modxper(k)=abs(xc);
    fasxper(k)=angle(xc);
end
